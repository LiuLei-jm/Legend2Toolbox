  ...
  <div class="app-container">
    <el-card header="脚本套管理">
      ...
      <el-table
        ...
        @current-change="handleSelectSet"
      >
      ...
      <el-pagination
        ...
        class="pagination"
        @current-change="fetchScriptSets"
      />
    </el-card>
    ...
    <el-card v-if="currentSet" class="detail-card">
      <template #header>
        <span
          >当前选中的脚本套:<strong>{{ currentSet.name }}</strong></span
        >
      </template>
...
const handleSaveSet = async () => {
  ...
  try {
    if (isEdit.value) {
      ...
      await ScriptSetService.putApiScriptsSets(setForm.value.id, requestData);
