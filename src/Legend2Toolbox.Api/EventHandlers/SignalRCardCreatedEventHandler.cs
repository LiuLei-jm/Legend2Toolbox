...
public class SignalRCardCreatedEventHandler : INotificationHandler<CardNumberCreatedEvent>
{
    ...
    public async Task Handle(CardNumberCreatedEvent notification, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning("无法向用户 {UserName} 推送卡号创建命令，原因：未找到该用户的 ConnectionKey.", notification.UserName);
            return;
        }
        ...
        var connections = _connectionManager.GetConnection(connectionKeyEntity.Key).ToList();
        ...
        if (connectionIds.Any())
        {
            ...
            _logger.LogInformation("已向用户 {UserName} 的 {Count} 个在线设备下发创建卡号 {cdk} 命令。", notification.UserName,
