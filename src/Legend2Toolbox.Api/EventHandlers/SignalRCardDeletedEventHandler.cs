...
public class SignalRCardDeletedEventHandler : INotificationHandler<CardNumberDeletedEvent>
{
    ...
    public SignalRCardDeletedEventHandler(IHubContext<ResourceSyncHub> hubContext, IConnectionManager connectionManager,
        IApplicationDbContext context, ILogger<SignalRCardDeletedEventHandler> logger)
    {
    ...
    {
        var connectionKeyEntity = await _context.ConnectionKeys.AsNoTracking()
            .FirstOrDefaultAsync(s => s.UserId == notification.UserId, cancellationToken);
        if (connectionKeyEntity is null)
        {
            _logger.LogWarning("无法向用户 {UserName} 推送卡号删除命令，原因：未找到该用户的 ConnectionKey.", notification.UserName);
            return;
        }
        ...
        var connections = _connectionManager.GetConnection(connectionKeyEntity.Key);
        ...
        if (connectionIds.Any())
        {
            ...
            _logger.LogInformation("已向用户 {UserName} 的 {Count} 个在线设备下发删除卡号 {cdk} 命令。", notification.UserName,
                connectionIds.Count, notification.Cdk);
        }
        else
        {
